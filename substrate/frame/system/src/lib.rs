...
impl<T: Config> Pallet<T> {
	...
	/// Set extrinsic count directly without per-extrinsic bookkeeping.
	///
	/// Used for minimal Bitcoin block execution where transactions are no-ops.
	pub fn set_extrinsic_count(count: u32) {
		ExtrinsicCount::<T>::put(count);
	}

	/// Set total extrinsics length directly.
	///
	/// Used for minimal Bitcoin block execution where transactions are no-ops.
	pub fn set_all_extrinsics_len(len: u32) {
		AllExtrinsicsLen::<T>::put(len);
	}
